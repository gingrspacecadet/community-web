...
export async function onRequestGet(context) {
  ...
  try {
    ...
    return new Response("Failed to get OAuth config: " + e.message, {
      status: 500,
    });
  }
  ...
  if (env.DB && typeof env.DB.prepare === "function") {
    try {
      ...
      return new Response("Failed to store user data: " + err.message, {
        status: 500,
      });
    }
  }
  ...
  return new Response(null, {
    ...
    headers: [
      [
        "Set-Cookie",
        `authToken=${tokenData.access_token}; Path=/; HttpOnly; ${config.cookies}SameSite=Lax; Max-Age=86400`
      ],
      [
        "Set-Cookie",
        `username=${encodeURIComponent(userData.username)}; Path=/; SameSite=Lax; Max-Age=86400`
      ],
      ["Location", "/pages/app"]
    ]
  });
